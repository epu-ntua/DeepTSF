...
@multi_asset(
    ...
    ins={'start_pipeline_run': AssetIn(key='start_pipeline_run', dagster_type=str),
         'load_raw_data_out': AssetIn(key='load_raw_data_out', dagster_type=dict)},
    outs={"etl_out": AssetOut(dagster_type=dict)})

def etl_asset(context, start_pipeline_run, load_raw_data_out):
    ...
    infered_resolution_series = load_raw_data_out["infered_resolution_series"]
    infered_resolution_past = load_raw_data_out["infered_resolution_past"]
    infered_resolution_future = load_raw_data_out["infered_resolution_future"]
    past_covs_uri = load_raw_data_out["past_covs_uri"]
    ...
    a = config.a
    wncutoff = config.wncutoff
    ycutoff = config.ycutoff
    ydcutoff = config.ydcutoff
    multiple = config.multiple
    imputation_method = config.imputation_method
    order = config.order
    rmv_outliers = config.rmv_outliers
    convert_to_local_tz = config.convert_to_local_tz
